...
function ensureValidFormat(fmt) {
  ...
  const normalized = v === "JPG" ? "JPEG" : v;
  if (!["PNG", "JPEG", "WEBP", "TIFF"].includes(normalized)) {
    throw new Error("Format must be PNG, JPEG, WEBP, or TIFF.");
  }
  return normalized;
}
...
async function makeZip(tmpDir, zipName, fileNames) {
  ...
  return new Promise((resolve, reject) => {
    ...
    output.on("error", (err) => {
      if (!resolved) {
        resolved = true;
        reject(new Error(`ZIP output stream error: ${err.message}`));
      }
    });
    ...
    archive.on("warning", (err) => {
      if (err.code !== "ENOENT") {
        console.warn(`[makeZip] Warning: ${err.message}`);
      }
    });
    ...
    for (const name of fileNames) {
      ...
      try {
        ...
        reject(new Error(`Failed to add file to ZIP: ${name} - ${err.message}`));
        return;
      }
    }
...
async function cmd_convert(payload) {
  ...
  const pngNames = Array.isArray(renderResult) ? renderResult : (renderResult.names || []);
  if (!pngNames || !Array.isArray(pngNames) || pngNames.length === 0) {
    throw new Error("PDF renderer failed to produce any image pages.");
  }
  ...
  for (const pngName of pngNames) {
    ...
    switch (fmt) {
      case "JPEG":
        outName = pngName.replace(/\.png$/, ".jpg");
        outPath = path.join(tmpDir, outName);
        await sharp(pngPath).jpeg({ quality: 92 }).toFile(outPath);
        await fsp.unlink(pngPath).catch(() => {});
        break;
      case "WEBP":
        outName = pngName.replace(/\.png$/, ".webp");
        outPath = path.join(tmpDir, outName);
        await sharp(pngPath).webp({ quality: 80 }).toFile(outPath);
        await fsp.unlink(pngPath).catch(() => {});
        break;
      case "TIFF":
        outName = pngName.replace(/\.png$/, ".tiff");
        outPath = path.join(tmpDir, outName);
        await sharp(pngPath).tiff({ compression: "lzw" }).toFile(outPath);
        await fsp.unlink(pngPath).catch(() => {});
        break;
      default:
        // PNG - keep the rendered file as-is
        outName = pngName;
        outPath = pngPath;
    }
  ...
  const zipPath = await makeZip(tmpDir, zipName, imageNames);
  ...
  try {
    ...
    throw new Error(`ZIP file verification failed: ${err.message}`);
  }
...
async function attemptCompressPdf({ inputPath, tmpDir, profile }) {
  ...
  if (!pngFiles || pngFiles.length === 0) {
    throw new Error(`PDF compression failed: Renderer produced no pages for profile ${profile.name}.`);
  }
...
async function cmd_convert_image_format(payload) {
  ...
  const webpQuality = Math.max(50, Math.min(100, Number(payload.webp_quality) || 80));
  const dpi = Number(payload.dpi ?? 150);
  ensureValidDpi(dpi);

  const normalized = targetFormat === "JPG" ? "JPEG" : targetFormat;
  if (!["PNG", "JPEG", "WEBP", "TIFF"].includes(normalized)) {
    throw new Error("Target format must be PNG, JPEG, WEBP, or TIFF.");
  }

  const sourceExt = path.extname(inputPath).toLowerCase();
  if (![".png", ".jpg", ".jpeg", ".webp", ".tiff", ".tif"].includes(sourceExt)) {
    throw new Error("Supported input formats: PNG, JPEG, WEBP, TIFF.");
  }

  // Determine source format
  let sourceFormat = "JPEG";
  if (sourceExt === ".png") sourceFormat = "PNG";
  else if (sourceExt === ".webp") sourceFormat = "WEBP";
  else if (sourceExt === ".tiff" || sourceExt === ".tif") sourceFormat = "TIFF";
  ...
  // Determine output extension and file name
  let outExt;
  switch (normalized) {
    case "JPEG":
      outExt = ".jpg";
  ...
  if ((normalized === "JPEG" || normalized === "TIFF") && metadata.hasAlpha) {
    processImage = image.flatten({ background: { r: 255, g: 255, b: 255 } });
  }

  // Convert to target format
  switch (normalized) {
    case "JPEG":
      await processImage.jpeg({ quality: jpegQuality, progressive: true, mozjpeg: true }).toFile(outPath);
      break;
    case "WEBP":
      await processImage.webp({ quality: webpQuality }).toFile(outPath);
      break;
    case "TIFF":
      await processImage.tiff({ compression: "lzw" }).toFile(outPath);
      break;
    default:
      // PNG
      await processImage.png({ compressionLevel: 9 }).toFile(outPath);
  }
