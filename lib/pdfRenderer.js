import { execFile } from "node:child_process";
import path from "node:path";

// Force tracing of pdfjs-dist for deployment
// @ts-ignore
// import * as _pdfjs from "pdfjs-dist"; // Removing this as it crashes Node.js without DOMMatrix polyfill

export async function renderPdfToPngs({ inputPath, tmpDir, pages, dpi }) {
  const script = path.join(process.cwd(), "scripts", "render-pdf.mjs");
  const config = {
    inputPath,
    outDir: tmpDir,
    pages,
    dpi,
  };

  const startTime = Date.now();
  return new Promise((resolve, reject) => {
    execFile(
      process.execPath,
      [script, JSON.stringify(config)],
      { maxBuffer: 10 * 1024 * 1024 },
      (err, stdout, stderr) => {
        const duration = Date.now() - startTime;
        if (err) {
          const message = stderr || err.message || "Unknown error";
          console.error(`[pdfRenderer] Process failed: ${message}`);
          return reject(new Error(`PDF rendering failed: ${message}`));
        }
        try {
          const parsed = JSON.parse(stdout);
          const result = {
            names: parsed.names ?? [],
            renderTime: parsed.renderTime ?? 0,
            totalTime: duration,
          };
          console.log(`[pdfRenderer] Rendered ${result.names.length} page(s) in ${duration}ms (script: ${result.renderTime}ms)`);
          resolve(result);
        } catch (parseErr) {
          reject(new Error(`Failed to parse render output: ${parseErr.message}\nOutput: ${stdout}`));
        }
      }
    );
  });
}
